...
export function HomePageClient({ initialData }: { initialData: HomepageData }) {
  ...
  const {
    marketEntries,
    heroBanners,
    latestEntries: allLatestEntries,
  } = initialData;
  ...
  const marketEntryById = useMemo(
    () => new Map(marketEntries.map((entry) => [entry.id, entry])),
    [marketEntries],
  );
  const filteredEntries = useMemo(() => {
    ...
    return marketEntries.filter((entry) => {
      const matchesCategory = matchesCategoryFilter(entry.category, category);
      const matchesQuery = !query || entry.searchText.includes(query);
      return matchesCategory && matchesQuery;
    });
  }, [category, marketEntries, search]);

  const filteredLatestEntries = useMemo(() => {
    const query = search.trim().toLowerCase();

    return allLatestEntries
      .map((entry) => {
        const marketEntry = marketEntryById.get(entry.marketEntryId);
        return marketEntry ? { ...entry, marketEntry } : null;
      })
      .filter((entry): entry is NonNullable<typeof entry> => Boolean(entry))
      .filter((entry) => {
      const matchesCategory = matchesCategoryFilter(entry.category, category);
      const matchesQuery = !query || entry.marketEntry.searchText.includes(query);
      return matchesCategory && matchesQuery;
    });
  }, [allLatestEntries, category, marketEntryById, search]);
              ...
              <section className="space-y-4 sm:space-y-6">
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-6 md:grid-cols-3 lg:grid-cols-5 lg:gap-x-4 lg:gap-y-8">
                  {latestEntries.map((entry) => (
                    <BottleMarketCard
                      key={`latest-${entry.id}`}
                      href={entry.marketEntry.href}
                      imageUrl={entry.marketEntry.imageUrl}
                      name={entry.marketEntry.name}
                      priceUsd={entry.marketEntry.priceUsd}
                      listingCount={entry.marketEntry.listingCount}
                    />
                  ))}
                </div>
              </section>
